...
def get_model_transfer_learning(model_name="resnet18", n_classes=50):
    ...
    # Customize classifier (FC layer) at the end
    # 1. Get number of input features of the last FC layer
    if model_name.startswith("resnet"):
        num_ftrs = model_transfer.fc.in_features
        model_transfer.fc = nn.Linear(num_ftrs, n_classes)
    else:
        raise NotImplementedError(f"Transfer learning is only supported for resnet models in this template.")
